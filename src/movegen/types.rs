...
impl Piece {
    pub fn name(&self) -> char {
        const PIECE_NAMES: [char; 6] = ['p', 'n', 'b', 'r', 'q', 'k'];
        if *self == Piece::None {
            return '.';
        }
...
pub mod square {
    ...
    #[inline]
    pub const fn perspective_sqr (square: Square, color: Color) -> Square {
        const FLIP_AMOUNT: [u8; 2] = [0, 56];
        square ^ FLIP_AMOUNT[color as usize]
    }
    #[inline]
